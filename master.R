...
datastory_name <- "snsf_gender_statistics"
...
# characters etc.)
datastory_name_de <- "frauen-unterrepraesentiert-oder-unterfinanziert"
datastory_name_en <- "women-underrepresented-or-underfunded"
datastory_name_fr <- "femmes-sous-representees-ou-sous-financees"

# English title and lead of the story (Mandatory, even if no EN version)
title_en <- "Gender monitoring (part 1): how women fare in SNSF project funding"
lead_en <- "The Swiss research landscape suffers from a chronic underrepresentation of women. This can also be seen in the share of women applying for funding at the SNSF. But how did this share evolve over time? And have women been less successful to raise funds?"
# TODO: adapt French (and German) title
# German title and lead of the story (Mandatory, even if no DE version)
title_de <- "Gender-Monitoring (Teil 1): So schneiden Frauen in der SNF-Projektförderung ab" 
lead_de <- "Frauen sind in der Forschungslandschaft der Schweiz chronisch untervertreten. Das zeigt sich auch am Anteil der Frauen, die ein Gesuch um Projektförderung beim SNF einreichen. Aber wie hat sich dieser Anteil im Laufe der Zeit entwickelt? Und waren Frauen bei der Gesuchstellung weniger erfolgreich als Männer?" 
# French title and lead of the story (Mandatory, even if no FR version)
title_fr <- "Gender monitoring (1) : la place des femmes dans l’encouragement de projets FNS"
lead_fr <- "Le paysage de la recherche suisse souffre d’une sous-représentation chronique des femmes. Cela se voit également dans la part des femmes qui demandent un financement au FNS. Mais comment cette part a-t-elle évolué au fil du temps? Et les femmes ont-elles moins bien réussi à lever des fonds?!" 
# Contact persons, always (first name + last name)
contact_person <- c("Rachel Heyard", "Manuela Ott")
...
publication_date <- "2021-10-28 02:00:00"
...
doi <- "https://doi.org/10.46446/datastory.women-underrepresented-or-underfunded" 
# URL to Github page (optional)
github_url <- "https://github.com/snsf-data/datastory_women_underrepresented_or_underfunded" 
# Put Tag IDs here. Only choose already existing tags. 
tags_ids <- c(270, # Gender monitoring series
              60, # Gender equality  
              80, # Project funding
              220, # Monitoring
              240) # Success rates
